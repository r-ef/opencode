...
import { createSeleneClient } from "@selene-ai/sdk"
...
const { client, server } = createSelene()
...
try {
  ...
  console.log("selene session", session.id)
  if (shareId) {
...
function createSelene() {
  ...
  const proc = spawn(`selene`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createSeleneClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/selene|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/selene` or `/oc`")
  }
}
...
async function assertSeleneConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to selene server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.selene.run" : "https://selene.run"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.selene.run/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("selene-github-action")
    response = await fetch("https://api.selene.run/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/selene" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/selene") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to selene...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "selene-agent[bot]"`
  await $`git config --global user.email "selene-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `selene/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/selene-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[selene session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
