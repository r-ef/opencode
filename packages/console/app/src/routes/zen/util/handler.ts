...
import { and, Database, eq, isNull, lt, or, sql } from "@selene-ai/console-core/drizzle/index.js"
import { KeyTable } from "@selene-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@selene-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@selene-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@selene-ai/console-core/util/date.js"
import { Identifier } from "@selene-ai/console-core/identifier.js"
import { Billing } from "@selene-ai/console-core/billing.js"
import { Actor } from "@selene-ai/console-core/actor.js"
import { WorkspaceTable } from "@selene-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@selene-ai/console-core/model.js"
import { Subscription } from "@selene-ai/console-core/subscription.js"
import { BlackData } from "@selene-ai/console-core/black.js"
import { UserTable } from "@selene-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@selene-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@selene-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@selene-ai/console-core/lite.js"
import { Resource } from "@selene-ai/console-resource"
...
export async function handler(
  ...
  const ADMIN_WORKSPACES = [
    ...
    "wrk_01K6W1A3VE0KMNVSCQT43BG2SX", // selene bench
  ]
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-selene-session") ?? ""
    const requestId = input.request.headers.get("x-selene-request") ?? ""
    const projectId = input.request.headers.get("x-selene-project") ?? ""
    const ocClient = input.request.headers.get("x-selene-client") ?? ""
    logger.metric({
      is_tream: isStream,
      session: sessionId,
      request: requestId,
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-selene-request")
          headers.delete("x-selene-session")
          headers.delete("x-selene-project")
          headers.delete("x-selene-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    const billingUrl = `https://selene.run/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://selene.run/workspace/${authInfo.workspaceID}/members`
