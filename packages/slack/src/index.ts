...
import { createSelene, type ToolPart } from "@selene-ai/sdk"
...
const selene = await createSelene({
  port: 0,
})
console.log("✅ Selene server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await selene.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new selene session...")
    const { client, server } = selene
    ...
    console.log("✅ Created selene session:", createResult.data.id)
  ...
  console.log("📝 Sending to selene:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Selene response:", JSON.stringify(result, null, 2))
