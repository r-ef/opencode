...
const managedConfigDir = process.env.SELENE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "selene.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "selene.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://selene.run/config.json",
        model: "test/model",
        username: "testuser",
      })
    },
...
test("ignores legacy tui keys in selene config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://selene.run/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "selene.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "selene.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .selene directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".selene")
...
test("loads agents from .selene/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".selene")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .selene/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".selene")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .selene/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".selene")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only SELENE_CONFIG_DIR", async () => {
  ...
  const prev = process.env.SELENE_CONFIG_DIR
  process.env.SELENE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.SELENE_CONFIG_DIR
    else process.env.SELENE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable SELENE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "selene.json"),
        JSON.stringify({ $schema: "https://selene.run/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "selene.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .selene config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".selene")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .selene config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "selene.json"),
        JSON.stringify({
          $schema: "https://selene.run/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .selene config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "selene.json"),
        JSON.stringify({
          $schema: "https://selene.run/config.json",
...
// Note: preload.ts sets SELENE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .selene config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .selene directory config enables it
      const opencodeDir = path.join(dir, ".selene")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "selene.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/selene")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/selene")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-selene@2.4.3")).toBe("oh-my-selene")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@selene/plugin@2.0.0")).toBe("@selene/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-selene@2.4.3", "file:///project/.selene/plugin/oh-my-selene.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.selene/plugin/oh-my-selene.js")
  })
  ...
  test("local plugin directory overrides global selene.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const opencodeDir = path.join(projectDir, ".selene")
...
describe("SELENE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["SELENE_DISABLE_PROJECT_CONFIG"]
    process.env["SELENE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["SELENE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["SELENE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .selene/ directories when flag is set", async () => {
    const originalEnv = process.env["SELENE_DISABLE_PROJECT_CONFIG"]
    process.env["SELENE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .selene directory with a command
          const opencodeDir = path.join(dir, ".selene", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .selene should NOT be in directories list
          const hasProjectSelene = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectSelene).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["SELENE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SELENE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["SELENE_CONFIG_DIR"]
      process.env["SELENE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["SELENE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["SELENE_CONFIG_DIR"]
      } else {
        process.env["SELENE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("SELENE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["SELENE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SELENE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["SELENE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["SELENE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from SELENE_CONFIG_DIR, not project
...
describe("SELENE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in SELENE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SELENE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["SELENE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://selene.run/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["SELENE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["SELENE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in SELENE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SELENE_CONFIG_CONTENT"]
