...
describe("Filesystem.contains", () => {
  ...
  test("blocks symlink target escaping parent", async () => {
    if (process.platform === "win32") return
    await using tmp = await tmpdir({
      init: async (dir) => {
        const outside = path.join(path.dirname(dir), `outside-${path.basename(dir)}.txt`)
        await Bun.write(outside, "outside")
        await fs.symlink(outside, path.join(dir, "escape-link.txt"))
      },
    })

    expect(Filesystem.contains(tmp.path, path.join(tmp.path, "escape-link.txt"))).toBe(false)
  })
})
...
describe("Instance.containsPath", () => {
  ...
  test("returns true for path inside worktree but outside directory (monorepo subdirectory scenario)", async () => {
    ...
    await Instance.provide({
      directory: subdir,
      fn: () => {
        // .selene at worktree root, but we're running from packages/lib
        expect(Instance.containsPath(path.join(tmp.path, ".selene", "state"))).toBe(true)
