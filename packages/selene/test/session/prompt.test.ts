...
import { DEFAULT_ANALYSIS_STEPS, SessionPrompt } from "../../src/session/prompt"
...
describe("session.prompt loop budgets", () => {
  test("uses a finite default budget for coordinated analysis", () => {
    const agent = {
      name: "build",
      mode: "primary",
      options: {},
    ...
    expect(SessionPrompt.stepLimit({ agent, coordinated: false })).toBe(Infinity)
    expect(SessionPrompt.stepLimit({ agent, coordinated: true })).toBe(DEFAULT_ANALYSIS_STEPS)
    expect(SessionPrompt.isFinalStep(DEFAULT_ANALYSIS_STEPS, DEFAULT_ANALYSIS_STEPS)).toBe(true)
    expect(
      SessionPrompt.stopAtLimit({
        step: DEFAULT_ANALYSIS_STEPS,
        max: DEFAULT_ANALYSIS_STEPS,
        result: "continue",
      }),
    ).toBe(true)
  })
  ...
  test("injects shared context updates from sibling sessions", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(text).toContain("add verification only where claims are still uncertain or high impact")
      },
    })
  })
  ...
  test("loop emits coordinator-owned synthesis once the plan is ready", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await SessionCoordinator.complete({
          ...
          text: [
            ...
            JSON.stringify({
              ...
              risks: [],
              verify_topics: ["topic-0", "topic-1", "topic-2"],
            }),
            "</analysis_json>",
          ].join("\n"),
        })
        ...
        expect(text).toContain("Verified findings")
        expect(text).toContain("statement-0")
        expect(text).toContain("statement-1")
      },
    })
  })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const other = await SessionPrompt.prompt({
            sessionID: session.id,
            agent: "build",
            model: { providerID: "selene", modelID: "kimi-k2.5-free" },
            noReply: true,
