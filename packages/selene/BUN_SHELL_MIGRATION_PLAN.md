...
Replace runtime Bun shell template-tag usage in `packages/selene/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/selene/src` is removed except:
