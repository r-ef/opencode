...
import type { AssistantMessage, Event, SeleneClient, SessionMessageResponse, ToolPart } from "@selene-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: SeleneClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `selene auth login` in the terminal",
        name: "Login with selene",
        id: "selene-login",
      }
    ...
    async unstable_forkSession(params: ForkSessionRequest): Promise<ForkSessionResponse> {
      ...
      try {
        ...
        if (!forked) {
          throw new Error("Branch session returned no data")
        }

        const sessionId = forked.id
        await this.sessionManager.load(sessionId, directory, mcpServers, model)

        log.info("branch_session", { sessionId, mcpServers: mcpServers.length })
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Selene stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const seleneProvider = providers.find((p) => p.id === "selene")
    if (seleneProvider) {
      if (seleneProvider.models["big-pickle"]) {
        return { providerID: "selene", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(seleneProvider.models))
      if (best) {
        return {
