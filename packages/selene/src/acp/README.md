...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for selene.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal selene sessions
...
selene acp --cwd /path/to/project
```
...
```bash
SELENE_ENABLE_QUESTION_TOOL=1 selene acp
```
...
- **Terminal Integration**: Full terminal support via selene's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | selene acp
```
...
ACP sessions map cleanly to selene's internal session model:
