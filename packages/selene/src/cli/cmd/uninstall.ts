...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall selene and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Selene")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g selene-ai",
      pnpm: "pnpm uninstall -g selene-ai",
      bun: "bun remove -g selene-ai",
      yarn: "yarn global remove selene-ai",
      brew: "brew uninstall selene",
      choco: "choco uninstall selene",
      scoop: "scoop uninstall selene",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall selene -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Selene!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# selene") || content.includes(".selene/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".selene/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".selene/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".selene"))
    ) {
      continue
    }
