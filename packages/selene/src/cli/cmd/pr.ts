...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run selene",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for selene session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found selene session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`selene import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting selene...")
        UI.println()

        // Launch selene TUI with session ID if available
        const { spawn } = await import("child_process")
        const cli = sessionId ? ["-s", sessionId] : []
        const proc = spawn("selene", cli, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
        ...
        await new Promise<void>((resolve, reject) => {
          proc.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`selene exited with code ${code}`))
          })
