...
import type { Path } from "@selene-ai/sdk"
...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const list = () =>
      sdk.client.session
        .list({ start: Date.now() - span })
        .then((x) => (x.data ?? []).toSorted((a, b) => a.id.localeCompare(b.id)))

    function fold<T extends { sessionID: string }>(items: T[]) {
      return items.reduce(
        (acc, item) => {
    ...
    async function reload() {
      const [session, status, permission, question] = await Promise.all([
        list(),
      ...
      batch(() => {
        ...
        setStore("permission", reconcile(fold(permission.data ?? [])))
        setStore("question", reconcile(fold(question.data ?? [])))
      })
    }

    async function bootstrap() {
      console.log("bootstrapping")
      const sessionListPromise = list()
      ...
      await Promise.all(blockingRequests)
        ...
        .then(() => {
          ...
          Promise.all([
            ...
            sdk.client.permission.list().then((x) => setStore("permission", reconcile(fold(x.data ?? [])))),
            sdk.client.question.list().then((x) => setStore("question", reconcile(fold(x.data ?? [])))),
    ...
    onMount(() => {
      ...
      const timer = setInterval(() => {
        ...
        reload().catch((error) => {
          Log.Default.warn("tui session refresh failed", {
