...
import type { AssistantMessage, Part, ToolPart, UserMessage, TextPart, ReasoningPart } from "@selene-ai/sdk/v2"
...
import type { TaskBranchTool, TaskTool } from "@/tool/task"
...
import { DialogBranches } from "./dialog-branches"
...
export function Session() {
  ...
  const subagents = createMemo(() => {
    ...
    return sync.data.session
      .filter((x) => x.kind === "subagent" && x.parentID === id)
  ...
  const siblings = createMemo(() => {
    ...
    if (!parentID) return []
    return sync.data.session
      .filter((x) => x.kind === "subagent" && x.parentID === parentID)
      .toSorted((a, b) => a.time.created - b.time.created || a.id.localeCompare(b.id))
  })
  const branches = createMemo(() => {
    ...
    if (!rootID) return []
    return sync.data.session
      .filter((x) => x.kind === "interactive" && x.rootID === rootID)
      .toSorted((a, b) => a.time.created - b.time.created || a.id.localeCompare(b.id))
  })
  ...
  // Handle initial prompt from branch
  createEffect(() => {
  ...
  function moveBranch(direction: number) {
    if (branches().length <= 1) return
    let next = branches().findIndex((x) => x.id === session()?.id) + direction
    if (next >= branches().length) next = 0
    if (next < 0) next = branches().length - 1
    const item = branches()[next]
    if (!item) return
    navigate({
      type: "session",
      sessionID: item.id,
  ...
  function childSessionHandler(func: (dialog: DialogContext) => void) {
    return (dialog: DialogContext) => {
      if (session()?.kind !== "subagent" || dialog.stack.length > 0) return
      func(dialog)
    }
  }
  ...
  command.register(() => [
    ...
    {
      title: "Branches",
      value: "session.branches",
      category: "Session",
      enabled: session()?.kind === "interactive",
      slash: {
        name: "branches",
      },
      onSelect: (dialog) => {
        dialog.replace(() => <DialogBranches sessionID={route.sessionID} />)
      },
    },
    {
    ...
    {
      title: "Go to root branch",
      value: "session.branch.root",
      category: "Session",
      hidden: true,
      enabled: session()?.kind === "interactive" && session()?.rootID !== session()?.id,
      onSelect: (dialog) => {
        const rootID = session()?.rootID
        if (rootID) {
          navigate({
    ...
    {
      title: "Next branch",
      value: "session.branch.next",
      category: "Session",
      hidden: true,
      enabled: session()?.kind === "interactive" && branches().length > 1,
      onSelect: (dialog) => {
        moveBranch(1)
        dialog.clear()
      },
        ...
        <box flexGrow={1} paddingBottom={1} paddingTop={0} paddingLeft={2} paddingRight={2} gap={1}>
          ...
          <Show when={session()}>
            ...
            <box flexShrink={0}>
              ...
              <Prompt
                visible={session()?.kind !== "subagent" && permissions().length === 0 && questions().length === 0}
                ref={(r) => {
                  prompt = r
                  promptRef.set(r)
                  // Apply initial prompt when prompt component mounts (e.g., from branch)
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.SELENE_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        <Match when={props.part.tool === "task_branch"}>
          <TaskBranch {...toolprops} />
        </Match>
...
function Task(props: ToolProps<typeof TaskTool>) {
  ...
  const rawLine = createMemo(() => {
    ...
    const reason = parts
      ...
      .at(-1)
      ?.text?.trim()
    if (reason) return `↳ ${Locale.truncate(reason.replace(/\s+/g, " "), 100)}`
  ...
  const content = createMemo(() => {
    ...
    if (spin()) {
      ...
      else if (current()) content.push(`↳ ${Locale.titlecase(current()!.tool)} ${(current()!.state as any).title}`)
      else if (tools().length > 0) content.push(`↳ ${tools().length} toolcalls`)
...
function TaskBranch(props: ToolProps<typeof TaskBranchTool>) {
  ...
  createEffect(() => {
    rows().forEach((item) => {
      if (!sync.data.message[item.sessionId]?.length) sync.session.sync(item.sessionId)
    })
  })
