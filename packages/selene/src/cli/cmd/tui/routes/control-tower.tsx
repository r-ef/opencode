...
import type { Message, PermissionRequest, QuestionInfo, QuestionRequest, Session, SessionStatus, TaskBranchRun, TaskEvent, Todo, ToolPart } from "@selene-ai/sdk/v2"
...
export function ControlTower() {
  ...
  createEffect(() => {
    ...
    const stopPlan = sdk.event.on("session.coordinator", (evt) => {
      const row = root()
      if (!row) return
      if (evt.properties.root_session_id !== row.info.rootID) return
      void refetchPlan()
    })
    onCleanup(stop)
    onCleanup(stopPlan)
