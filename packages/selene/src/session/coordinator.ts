...
const MAX_PRIMARY = 8
const DEFAULT_VERIFIER = 1

function expansive(query: string) {
  return /\b(codebase|repo(sitory)?|project|everything|everywhere|whole|entire|all)\b/i.test(query)
}

function requirements(query: string) {
  const match = query.match(/\b(\d+)\s*(?:sub-?agents?|agents?|workstreams?|parallel)\b/i)
  const base = expansive(query) ? 3 : DEFAULT_PRIMARY
  const requested = match ? Math.min(Math.max(parseInt(match[1], 10), base), MAX_PRIMARY) : base
...
export namespace SessionCoordinator {
  ...
  export const WorkStatus = z.enum(["planned", "running", "completed", "failed"])
  ...
  export const WorkInfo = z.object({
    ...
    metadata: z
      .object({
        ...
        verify_extra: z.array(z.string()).optional(),
  ...
  function accepted(input: ClaimInfo[]) {
    ...
    for (const item of input.filter((item) => ["verified", "resolved"].includes(item.status))) {
  ...
  function pending(input: ClaimInfo[]) {
    const map = new Map<string, ClaimInfo[]>()
    for (const item of input.filter((item) => item.status === "reported")) {
      const key = claimKey(item)
      const list = map.get(key) ?? []
      list.push(item)
      map.set(key, list)
  ...
  function hinted(input: WorkInfo[]) {
    return [...new Set(input.flatMap((item) => item.metadata?.verify_topics ?? []))]
  }

  function workStatus(input: string) {
    if (input === "verified" || input === "resolved") return "completed"
    if (input === "conflict" || input === "cancelled") return "failed"
    return input
  }
  ...
  function formatPrompt(input: { work: WorkInfo; query: string; extra?: string }) {
    return [
      ...
      `It is valid to return zero claims when the evidence is insufficient. Do not guess.`,
      `Only include claims that are direct observations supported by the cited lines you actually inspected.`,
  ...
  function verifier(plan: PlanInfo, works: WorkInfo[], claims: ClaimInfo[]) {
    const open = pending(claims).slice(0, 8)
    const topics = open.map((item) => item.topic)
    const extra = hinted(works)
      .filter((item) => !topics.includes(item))
      .slice(0, Math.max(0, 8 - topics.length))
    const evidence = open
    ...
    const row = {
      ...work(
        ...
        "verifier",
        "Verification",
        `Independently verify the primary analysis for: ${plan.query}`,
        "general",
        plan.root_session_id,
        plan.id,
        against[0],
    ...
    return {
      ...
      task: {
        ...
        prompt: formatPrompt({
          ...
          query: plan.query,
          extra: [
            "Independently verify the available findings and challenge weak claims.",
            topics.length ? `Topics:\n${topics.map((item) => `- ${item}`).join("\n")}` : "",
            extra.length ? `Additional focus areas:\n${extra.map((item) => `- ${item}`).join("\n")}` : "",
            evidence ? `Current claims:\n${evidence}` : "",
            topics.length ? "You must address every listed topic." : "Confirm whether the primary work missed any important contradictions or unsupported conclusions.",
            "Use verdict=confirm when a claim holds up independently and verdict=contradict when it does not.",
            "If the available evidence is too weak to support the reported statement, use verdict=contradict and say that directly.",
          ]
            .filter(Boolean)
            .join("\n\n"),
        }),
  ...
  async function publish(session_id: string) {
    const snapshot = await build(await active(session_id))
  ...
  async function exhausted(plan: PlanInfo) {
    ...
    const live = rows.some((item) => ["planned", "running", "completed"].includes(item.status))
    if (live) return false
    return rows.every((item) => item.status === "failed")
  }
  ...
  export const complete = fn(
    ...
    async (input) => {
      ...
      if (work.role === "verifier" && work.metadata?.verify_topics?.length) {
        const seen = new Set(next.map((item) => item.metadata.normalized))
        const miss = work.metadata.verify_topics.filter((item) => !seen.has(normalize(item)))
        if (miss.length) {
          Database.use((db) =>
            db
              .update(SessionCoordinatorWorkTable)
              .set({
                ...
                metadata: {
                  ...(work.metadata ?? {}),
                  summary: `Verification did not cover all requested topics: ${miss.join(", ")}`,
                  error: `Verification did not cover all requested topics: ${miss.join(", ")}`,
                  context_id: input.context_id,
  ...
  async function sync(session_id: string, emit: boolean) {
    ...
    const open = pending(nextClaims).length
        ...
        : bad.length > 0 || reconActive
          ...
          : primaryDone < plan.requirements.primary
            ? ws.some((item) => item.role === "primary" && ["running", "completed", "verified", "resolved"].includes(item.status))
              ? "running"
              : "planned"
            : open > 0 || verifierDone < plan.requirements.verifier || verifyActive
              ? "awaiting_verification"
              : "ready_to_finalize"
    const next = {
      ...plan,
    ...
    if (emit) await publish(session_id)
    return snapshot
  }
  ...
  export const refresh = fn(
    ...
    async (input) => sync(input.session_id, true),
  )
  ...
  function retry(plan: PlanInfo, row: WorkInfo) {
    ...
    const extra =
      row.role === "verifier"
        ? [
            row.metadata?.verify_topics?.length ? `Topics:\n${row.metadata.verify_topics.map((item) => `- ${item}`).join("\n")}` : "",
            row.metadata?.verify_extra?.length ? `Additional focus areas:\n${row.metadata.verify_extra.map((item) => `- ${item}`).join("\n")}` : "",
            row.metadata?.verify_topics?.length ? "You must address every listed topic." : "",
          ]
            .filter(Boolean)
            .join("\n\n")
        : undefined
  ...
  export const schedule = fn(
    ...
    async (input) => {
      ...
      const open = pending(await claims(plan.id)).length
      if (!bad.length && primaryDone >= plan.requirements.primary && (verifierDone < plan.requirements.verifier || open > 0) && !verifierLive) {
  ...
  export const reply = fn(z.object({ session_id: z.string() }), async (input) => {
    ...
    return [
      head,
      "",
      findings.length ? "**Verified findings**" : undefined,
      ...findings,
      !findings.length ? "No verified findings passed coordinator verification." : undefined,
