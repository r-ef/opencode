...
export const DEFAULT_ANALYSIS_STEPS = 10
...
export namespace SessionPrompt {
  ...
  export function stepLimit(input: { agent: Agent.Info; coordinated: boolean }) {
    if (input.agent.steps) return input.agent.steps
    if (input.coordinated) return DEFAULT_ANALYSIS_STEPS
    return Infinity
  }

  export function isFinalStep(step: number, max: number) {
    return Number.isFinite(max) && step >= max
  }

  export function stopAtLimit(input: {
    step: number
    max: number
    result: SessionProcessor.Result
    error?: MessageV2.Assistant["error"]
  }) {
    return isFinalStep(input.step, input.max) && input.result === "continue" && !input.error
  }
    ...
    return await Session.provide({
      ...
      fn: async (session) => {
        ...
        while (true) {
          ...
          if (task?.type === "subtask") {
            ...
            for (const t of subtasks) {
              ...
              await Session.updatePart({
                ...
                sessionID,
                type: "text",
                text: "Summarize the task tool output above. Continue only if more work is still needed.",
                synthetic: true,
          ...
          const maxSteps = stepLimit({
            agent,
            coordinated: Boolean(gate?.plan),
          })
          const isLastStep = isFinalStep(step, maxSteps)
          ...
          const tools: Record<string, AITool> = isLastStep
            ? {}
            : await resolveTools({
                agent,
                session,
            ...
            for (const msg of msgs) {
              ...
              for (const part of msg.parts) {
                ...
                part.text = [
                  ...
                  part.text,
                  "",
                  "Please address this message, then continue only if work still remains.",
                  "</system-reminder>",
                ].join("\n")
              }
            }
          ...
          if (nextGate?.plan && nextGate.ready && modelFinished && !processor.message.error) {
            if ((lastUser.format?.type ?? "text") === "text") {
              const text = await SessionCoordinator.reply({
                session_id: sessionID,
              }).catch(() => undefined)
              if (
          ...
          if (stopAtLimit({ step, max: maxSteps, result, error: processor.message.error })) break
          if (result === "stop") break
          if (result === "compact") {
            await SessionCompaction.create({
              sessionID,
      ...
      parts.unshift(
        assign({
          ...
          text: [
            ...
            "For broad analysis, compare the strongest conclusions, resolve material conflicts, and add verification only where claims are still uncertain or high impact.",
          ].join("\n"),
        }),
      )
    }
  ...
  async function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; session: Session.Info }) {
    ...
    if (!Flag.SELENE_EXPERIMENTAL_PLAN_MODE) {
