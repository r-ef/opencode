...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.SELENE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.SELENE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.SELENE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".selene") && dir !== Flag.SELENE_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old selene.json shape) still get their settings applied.
    const normalized = (() => {
