...
const TUI_SCHEMA_URL = "https://selene.run/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from selene.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const files = await seleneFiles(input)
...
async function seleneFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.SELENE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("selene", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "selene")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "selene"))
  }
  if (Flag.SELENE_CONFIG) files.push(Flag.SELENE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "selene"))
