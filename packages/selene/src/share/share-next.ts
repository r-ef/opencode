...
import type * as SDK from "@selene-ai/sdk/v2"
...
export namespace ShareNext {
  ...
  const disabled = process.env["SELENE_DISABLE_SHARE"] === "true" || process.env["SELENE_DISABLE_SHARE"] === "1"
  ...
  export async function create(sessionID: string) {
    ...
    void fullSync(sessionID).catch((error) => {
      log.error("full sync failed", { sessionID, error })
    })
    return result
  }
