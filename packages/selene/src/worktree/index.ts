...
export namespace Worktree {
  ...
  export async function ready(info: Info) {
    ...
    if (!booted) {
      throw new CreateFailedError({ message: "Failed to bootstrap worktree" })
    }
  ...
  export async function boot(info: Info, startCommand?: string) {
    const projectID = Instance.project.id
    const extra = startCommand?.trim()

    await ready(info)

    const ok = await runStartScripts(info.directory, { projectID, extra })
    if (!ok) {
      throw new StartCommandFailedError({ message: "Failed to run worktree start command" })
    }
  }
  ...
  export async function createFromInfo(info: Info, startCommand?: string) {
    ...
    return () => {
      void boot(info, startCommand).catch((error) => {
