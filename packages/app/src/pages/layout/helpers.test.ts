...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("selene://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("selene://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("selene://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("selene://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("selene://open-project")).toBeUndefined()
    expect(parseDeepLink("selene://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "selene://open-project?directory=/a",
      "selene://other?directory=/b",
      "selene://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("selene://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("selene://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("selene://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("selene://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "selene://new-session?directory=/a",
      "selene://open-project?directory=/b",
      "selene://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __SELENE__: {
        deepLinks: ["selene://open-project?directory=/a"],
      },
    } as unknown as Window & { __SELENE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["selene://open-project?directory=/a"])
