...
import { findLast } from "@selene-ai/util/array"
import { base64Encode } from "@selene-ai/util/encode"
import { extractPromptFromParts } from "@/utils/prompt"
import { UserMessage } from "@selene-ai/sdk/v2"
...
export const useSessionCommands = (actions: SessionCommandContext) => {
  ...
  const [rows, { refetch: refetchBranches }] = createResource(
    () => (params.id && info()?.kind === "interactive" ? params.id : undefined),
    async (sessionID) => {
  ...
  const jump = (item?: { id: string; directory: string }) => {
    if (!item) return
  ...
  const sessionActionCommands = createMemo(() => [
    ...
    sessionCommand({
      ...
      slash: "branch",
      disabled: !params.id || info()?.kind !== "interactive",
      onSelect: async () => {
        const sessionID = params.id
        if (!sessionID) return
    ...
    sessionCommand({
      id: "session.branches",
      title: "Branches",
      description: "Browse the branch family for this session",
      slash: "branches",
      disabled: !params.id || info()?.kind !== "interactive",
      onSelect: () => {
        const sessionID = params.id
    ...
    sessionCommand({
      id: "session.branch.root",
      title: "Go to root branch",
      description: "Open the root branch for this session family",
      disabled: !params.id || info()?.kind !== "interactive" || branches().length <= 1,
      onSelect: () => jump(branches().find((item) => item.id === info()?.rootID)),
    }),
    sessionCommand({
      id: "session.branch.next",
      title: "Next branch",
      description: "Open the next branch in this session family",
      disabled: !params.id || info()?.kind !== "interactive" || branches().length <= 1,
      onSelect: () => move(1),
    }),
    sessionCommand({
      id: "session.branch.previous",
      title: "Previous branch",
      description: "Open the previous branch in this session family",
      disabled: !params.id || info()?.kind !== "interactive" || branches().length <= 1,
      onSelect: () => move(-1),
    }),
  ])
