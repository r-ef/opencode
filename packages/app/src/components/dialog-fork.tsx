...
import type { TextPart as SDKTextPart } from "@selene-ai/sdk/v2/client"
...
export const DialogFork: Component = () => {
  ...
  const handleSelect = (item: ForkableMessage | undefined) => {
    ...
    sdk.client.session
      .branch({ sessionID, messageID: item.id })
      .then((branch) => {
        if (!branch.data) {
        ...
        dialog.close()
        navigate(`/${base64Encode(branch.data.directory)}/session/${branch.data.id}`)
