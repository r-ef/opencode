...
Možete dodati vanjske alate u Selene koristeći _Model Context Protocol_, ili MCP. Selene podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Selene Config](https://selene.run/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/selene`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Selene automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Selene će:
...
Ako server zahtijeva autentifikaciju, Selene će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `selene mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Selene sigurno čuva tokene u `~/.local/share/selene/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Selene, zajedno s ugrađenim alatima. Možete njima upravljati kroz Selene konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Selene sa vašim Sentry nalogom.
