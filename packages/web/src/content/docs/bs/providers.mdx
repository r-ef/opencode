---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Selene.
---

import config from "../../../../config.mjs"
export const console = config.console

Selene koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Selene konfiguraciji.
...
u `~/.local/share/selene/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Selene
config.
...
Selene Zen je lista modela koje je obezbedio Selene tim koji su bili
testirano i potvrđeno da dobro radi sa Selene. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Selene Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite selene i idite na [selene.run/auth](https://selene.run/auth).
...
Radi kao i svaki drugi provajder u Selene i potpuno je opcionalan za korištenje.

---

## Selene Go

Selene Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Selene tim i koji su testirani i verificirani da dobro rade s Selene-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Selene Go` i idite na [selene.run/auth](https://selene.run/zen).
...
Da biste koristili Amazon Bedrock s Selene:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete selene:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `selene.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Selene.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi selene ispravno radio.
   :::
...
Također možete dodati modele kroz svoju selene konfiguraciju.
...
da pohrani token u selene auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Selene koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Selene
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`selene.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [selene-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `selene.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s selene:
...
Za korištenje Google Vertex AI s Selene:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće selene.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [selene-helicone-session](https://github.com/H2Shami/selene-helicone-session) da automatski prijavite svaki Selene razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Selene razgovor naveden kao zasebna sesija.
...
Možete konfigurirati selene za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati selene za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati selene za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Selene. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/selene) za detalje.
:::
...
Da biste koristili Ollama Cloud s Selene:
...
3. Kopirajte API ključ za korištenje u Selene.
...
6. **Važno**: Prije upotrebe modela oblaka u Selene, morate lokalno povući informacije o modelu:
...
Selene Zen je lista testiranih i verifikovanih modela koju je obezbedio Selene tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Selene Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Selene Zen**.

```txt
   /connect
```

3. Unesite svoj Selene API ključ.
...
Također možete dodati dodatne modele putem vaše selene konfiguracije.
...
5. Također ih možete prilagoditi putem vaše selene konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Seleneom:
...
Također možete prilagoditi modele kroz svoju selene konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s selene-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `selene.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Selene da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `selene auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Selene konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj selene konfiguraciji.
