...
Postoje dvije vrste agenata u Selene: primarni agenti i podagenti.
...
Selene dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Selene dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Selene dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `selene.json`:
...
- Globalno: `~/.config/selene/agents/`
- Po projektu: `.selene/agents/`
...
Ako temperatura nije navedena, Selene koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Selene konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Selene konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Selene Zen](/docs/zen), koristili biste `selene/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `selene models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/selene).
:::
