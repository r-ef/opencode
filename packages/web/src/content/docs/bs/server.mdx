---
title: Server
description: Komunicirajte s selene serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `selene serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji selene klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `selene.local` |
...
Postavite `SELENE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `selene`, ili postavite `SELENE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `selene serve` i za `selene web`.
...
Kada pokrenete `selene`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite selene server za programsku interakciju sa selene-om.
:::

Ova arhitektura omogucava selene podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `selene serve` da startate standalone server. Ako je selene TUI vec pokrenut, `selene serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Selene [IDE](/docs/ide) pluginovi.
...
selene server izlaže sljedece API-je.
