...
U Selene mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Selene koristi nasu `selene` temu.
...
Selene dolazi sa vise ugradenih tema.
...
- Zele da Selene odgovara izgledu njihovog terminala
...
Selene podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/selene/themes/*.json` ili `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.selene/themes/*.json`
4. **Trenutni radni direktorij** - `./.selene/themes/*.json`
