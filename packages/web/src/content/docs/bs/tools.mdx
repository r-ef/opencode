...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Selene dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Selene.
...
:::note
Ovaj alat je dostupan samo kada je `SELENE_EXPERIMENTAL_LSP_TOOL=true` (ili `SELENE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Selene provajdera ili kada je varijabla `SELENE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Selene:
