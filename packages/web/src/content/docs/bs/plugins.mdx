---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Selene.
---

Dodaci vam omogućavaju da proširite Selene spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Selene.
...
- `.selene/plugins/` - Dodaci na nivou projekta
- `~/.config/selene/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/selene/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/selene/selene.json`)
2. Konfiguracija projekta (`selene.json`)
3. Globalni direktorij dodataka (`~/.config/selene/plugins/`)
4. Direktorij dodataka projekta (`.selene/plugins/`)
...
Selene pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Selene SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Selene.
...
Spriječite selene da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u selene:
...
Pomoćnik `tool` kreira prilagođeni alat koji selene može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za selene zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://selene.run/docs/sdk) za detalje.
