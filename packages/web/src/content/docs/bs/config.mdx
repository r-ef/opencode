---
title: Konfiguracija
description: Korištenje Selene JSON konfiguracije.
---

Možete konfigurirati Selene koristeći JSON konfiguracijski fajl.

---

## Format

Selene podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/selene`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/selene/selene.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`SELENE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`selene.json` u projektu) - postavke specifične za projekat
5. **`.selene` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`SELENE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.selene` i `~/.config/selene` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/selene` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Selene konfiguraciju u `~/.config/selene/selene.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/selene/tui.json`.
...
Dodajte `selene.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Selene pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `SELENE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `SELENE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.selene` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.selene`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`selene.run/config.json`**](https://selene.run/config.json).

TUI konfiguracija koristi [**`selene.run/tui.json`**](https://selene.run/tui.json).
...
Koristite `SELENE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `selene.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `selene serve` i `selene web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Selene server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `selene.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Selene konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Selene pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/selene/agents/` ili `.selene/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Selene će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`selene run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/selene/commands/` ili `.selene/commands/`. [Saznajte više ovdje](/docs/commands).
...
Selene će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Selene **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Selene sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.selene/plugins/` ili `~/.config/selene/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Selene da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
