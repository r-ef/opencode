...
Agent skills omogucavaju Selene da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Selene pretrazuje ove lokacije:

- Konfiguracija projekta: `.selene/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/selene/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Selene ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.selene/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/selene/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.selene/skills/git-release/SKILL.md` ovako:
...
Selene navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `selene.json`:
...
**Za ugradene agente** (u `selene.json`):
