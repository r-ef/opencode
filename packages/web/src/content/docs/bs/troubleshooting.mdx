...
Da biste otklonili probleme s Selene, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\selene\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `selene --log-level DEBUG`.

---

## Pohrana

Selene pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/selene/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\selene`
...
Selene Desktop pokreće lokalni Selene server (`selene-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Selene` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/selene/selene.jsonc` (ili `~/.config/selene/selene.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/selene/selene.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\selene\selene.jsonc`
...
Selene također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/selene/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\selene\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Selene mogao ponovo izgraditi.

1. Potpuno zatvorite Selene Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/selene`
- **Linux**: obrišite `~/.cache/selene` (ili pokrenite `rm -rf ~/.cache/selene`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\selene`

3. Ponovo pokrenite Selene Desktop.

---

### Rješavanje problema sa vezom na serveru

Selene Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `selene.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `SELENE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `SELENE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Selene Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Selene karakteristikama.

---

### Obavještenja se ne prikazuju

Selene Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Selene u postavkama vašeg OS-a, i
...
1. Zatvorite Selene Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Selene Desktop aplikacije):

- `selene.settings.dat` (URL zadanog servera za desktop)
- `selene.global.dat` i `selene.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `selene models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\selene`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Selene dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\selene`

2. Ponovo pokrenite Selene da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Selene da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Selene će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
