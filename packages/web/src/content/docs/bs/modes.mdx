...
ove ili konfigurirajte svoje putem selene config.
...
selene dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.selene/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `selene.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/selene/modes/`
- Projekat: `.selene/modes/`
...
Ako temperatura nije navedena, selene koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.selene/modes/` za specifične načine rada ili `~/.config/selene/modes/` za globalne načine:
