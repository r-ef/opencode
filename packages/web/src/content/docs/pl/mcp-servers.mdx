...
Możesz dodać dodatkowe informacje do selene za pomocą _Model Context Protocol_, w skrócie MCP. selene obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [selene Config](https://selene.run/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/selene`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
selene automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, selene:
...
Jeśli serwer wymaga uwierzytelnienia, selene poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `selene mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji selene bezpieczne przechowa tokeny w `~/.local/share/selene/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w selene, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie selene, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć selene z kontem Sentry.
