---
title: Serwer
description: Interakcja z serwerem selene za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `selene serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient selene.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `selene.local` |
...
Ustaw `SELENE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `selene` lub ustaw `SELENE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `selene serve`, jak i `selene web`.
...
Kiedy uruchomisz `selene`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera selene do programowej interakcji z selene.
:::

Ta architektura umożliwia obsługę wielu klientów przez selene i programową interakcję z kodem otwartym.

Możesz uruchomić `selene serve`, aby uruchomić samodzielny serwer. Jeśli masz
selene TUI działa, `selene serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki selene [IDE](/docs/ide).
...
Serwer selene udostępnia następujące interfejsy API.
