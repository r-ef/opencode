---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć selene.
---

Wtyczki umożliwiające wyłączenie selene poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania selene.
...
- `.selene/plugins/` - Wtyczki na poziomie projektu
- `~/.config/selene/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/selene/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/selene/selene.json`)
2. Konfiguracja projektu (`selene.json`)
3. Globalny katalog wtyczek (`~/.config/selene/plugins/`)
4. Katalog wtyczek projektu (`.selene/plugins/`)
...
selene uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z selene do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do selene.
...
:::note
Jeśli korzystasz z aplikacji komputerowej selene, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj selene czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do selene:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod selene. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://selene.run/docs/sdk).
