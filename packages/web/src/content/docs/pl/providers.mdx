---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w selene.
---

import config from "../../../../config.mjs"
export const console = config.console

selene używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji selene.
...
w `~/.local/share/selene/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim selene
config.
...
Selene Zen to lista modeli dostarczonych przez zespół selene, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z selene. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Selene Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Selene Zen` i przejdź do [selene.run/auth](https://selene.run/zen).
...
Działa jak każdy inny dostawca w selene i jest całkowicie opcjonalny w użyciu.

---

## Selene Go

Selene Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół selene, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z selene.

1. Uruchom polecenie `/connect` w TUI, wybierz `Selene Go` i przejdź do [selene.run/auth](https://selene.run/zen).
...
Aby używać Amazon Bedrock z selene:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania selene:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `selene.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w selene nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod selene działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji selene.
...
selene używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować selene
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`selene.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [selene-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracja

Customize through `selene.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem selene:
...
Aby używać Google Vertex AI z selene:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania selene.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą selene, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [selene-helicone-session](https://github.com/H2Shami/selene-helicone-session), aby automatycznie rejestrować każdą konwersację selene jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja selene będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować selene tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować selene tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować selene tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem selene. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/selene).
:::
...
Aby korzystać z Ollama Cloud z selene:
...
3. Skopiuj klucz API do użycia w selene.
...
6. **Ważne**: Przed użyciem modeli chmurowych w selene musisz pobrać informacje o modelu lokalnie:
...
Selene Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół selene. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Selene Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Selene Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API selene.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji selene.
...
5. Możesz je także dostosować za pomocą konfiguracji selene. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z selene:
...
Możesz także dostosować modele za pomocą konfiguracji selene. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z selene. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `selene.json` w katalogu projektu:
...
Pola `limit` pozwalają selene zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `selene auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację selene i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji selene.
