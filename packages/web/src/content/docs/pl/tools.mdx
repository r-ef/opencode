...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Selene posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Selene.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `SELENE_EXPERIMENTAL_LSP_TOOL=true` (lub `SELENE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Selene lub gdy zmienna środowiskowa `SELENE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Selene:
