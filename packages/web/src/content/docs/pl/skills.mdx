...
Umiejętności agenta pozwalają selene odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
selene przeszukuje te lokalizacje:

- Project config: `.selene/skills/<name>/SKILL.md`
- Global config: `~/.config/selene/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu selene przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.selene/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/selene/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.selene/skills/git-release/SKILL.md` w ten sposób:
...
selene wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `selene.json`:
...
**Dla agentów wbudowanych** (w `selene.json`):
