...
W Selene obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Selene zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Selene ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Selene ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `selene.json`:
...
- Globalnie: `~/.config/selene/agents/`
- W projekcie: `.selene/agents/`
...
Jeśli nie zostanie skonfigurowana, Selene używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Selene, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Selene w `provider/model-id`. Na przykład, [Selene Zen](/docs/zen), użyj `selene/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `selene models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/selene).
:::
