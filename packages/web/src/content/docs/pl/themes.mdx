...
Dzięki selene możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie selene używa naszego własnego motywu `selene`.
...
selene ma kilka wbudowanych motywów.
...
- Chcą, aby Selene pasował do wyglądu ich terminala
...
selene obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/selene/themes/*.json` lub `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.selene/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.selene/themes/*.json`
