...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji selene.
...
te lub skonfiguruj własne za pomocą konfiguracji selene.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.selene/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `selene.json`:
...
Jeśli nie ma wpływu na temperaturę, selene stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna selene, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.selene/modes/` dla trybów zapisanych dla projektu lub `~/.config/selene/modes/` dla trybów globalnych:
