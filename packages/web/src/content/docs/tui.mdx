---
title: TUI
description: Using the Selene terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Selene provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Selene starts the TUI for the current directory.
...
When using the Selene TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Selene. Allows you to select from available providers and add their API keys.
...
This is separate from `selene.json`, which configures server/runtime behavior.
...
Use `SELENE_TUI_CONFIG` to load a custom TUI config path.
