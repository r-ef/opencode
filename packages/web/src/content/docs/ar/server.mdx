---
title: الخادم
description: تفاعل مع خادم selene عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `selene serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل selene استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `selene.local` |
...
عيّن `SELENE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `selene`، أو عيّن `SELENE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `selene serve` و `selene web`.
...
عند تشغيل `selene` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم selene للتفاعل مع selene برمجيا.
:::

تتيح هذه البنية لـ selene دعم عدة عملاء وتمكّنك من التفاعل مع selene برمجيا.

يمكنك تشغيل `selene serve` لبدء خادم مستقل. إذا كانت واجهة selene في terminal (TUI)
قيد التشغيل، فسيبدأ `selene serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Selene لـ [IDE](/docs/ide).
...
يعرض خادم selene واجهات API التالية.
