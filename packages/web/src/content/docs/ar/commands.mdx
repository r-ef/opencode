...
أنشئ الملف `.selene/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Selene أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Selene:
...
- على مستوى النظام: `~/.config/selene/commands/`
- لكل مشروع: `.selene/commands/`
...
يتضمن selene عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
