...
تتيح مهارات الوكيل لـ Selene اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Selene في هذه المواقع:

- إعدادات المشروع: `.selene/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/selene/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Selene صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.selene/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/selene/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.selene/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Selene المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `selene.json`:
...
**للوكلاء المدمجين** (في `selene.json`):
