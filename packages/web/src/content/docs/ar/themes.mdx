...
مع Selene يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Selene سمتنا `selene`.
...
- يريدون أن يطابق Selene مظهر terminal لديهم
...
يدعم Selene نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/selene/themes/*.json` أو `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.selene/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.selene/themes/*.json`
