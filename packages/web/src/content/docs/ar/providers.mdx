---
title: المزوّدون
description: استخدام أي موفّر LLM في Selene.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Selene ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Selene.
...
في `~/.local/share/selene/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Selene.
...
Selene Zen هي قائمة نماذج يوفّرها فريق Selene وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Selene. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Selene Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر selene، ثم انتقل إلى [selene.run/auth](https://selene.run/auth).
...
يعمل مثل أي مزوّد آخر في Selene واستخدامه اختياري بالكامل.

---

## Selene Go

Selene Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Selene، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Selene.

1. شغّل الأمر `/connect` في TUI، واختر `Selene Go`، ثم انتقل إلى [selene.run/auth](https://selene.run/zen).
...
لاستخدام Amazon Bedrock مع Selene:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل selene:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `selene.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Selene غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل selene بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات selene.
...
تخزين الرمز في مساحة تخزين مصادقة selene.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Selene نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Selene
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`selene.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [selene-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### التهيئة

خصّص عبر `selene.json`:
...
لاستخدام اشتراك GitHub Copilot مع selene:
...
لاستخدام Google Vertex AI مع Selene:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر selene، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [selene-helicone-session](https://github.com/H2Shami/selene-helicone-session) لتسجيل كل محادثة Selene تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Selene مدرجة كجلسة مستقلة.
...
يمكنك ضبط selene لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط selene لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط selene لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Selene. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/selene) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Selene:
...
3. انسخ مفتاح API لاستخدامه في Selene.
...
6. **مهم**: قبل استخدام نماذج السحابة في Selene، يجب سحب معلومات النموذج محليا:
...
Selene Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Selene. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Selene Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Selene Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Selene.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات selene.
...
5. يمكنك أيضا تخصيصها عبر إعدادات selene. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Selene:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات selene. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع selene. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `selene.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Selene بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `selene auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات selene وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات selene.
