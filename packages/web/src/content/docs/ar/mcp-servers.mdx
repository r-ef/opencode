...
يمكنك إضافة أدوات خارجية إلى Selene باستخدام _Model Context Protocol_، أو MCP. يدعم Selene خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Selene](https://selene.run/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/selene` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Selene تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Selene بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Selene المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `selene mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Selene الرموز بشكل آمن في `~/.local/share/selene/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Selene إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Selene مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Selene بحساب Sentry الخاص بك.
