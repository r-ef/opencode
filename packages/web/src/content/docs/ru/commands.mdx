...
Создайте `.selene/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию selene или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем selene [config](/docs/config):
...
- Глобальный: `~/.config/selene/commands/`
- Для каждого проекта: `.selene/commands/`
...
selene включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
