...
selene ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak selene kendi `selene` temasını kullanır.
...
selene birden fazla yerleşik temayla gelir.
...
- selene'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
selene, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/selene/themes/*.json` veya `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.selene/themes/*.json`
4. **Geçerli çalışma dizini** - `./.selene/themes/*.json`
