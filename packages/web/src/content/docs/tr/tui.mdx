---
title: TUI
description: Selene terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Selene, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Selene'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Selene TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Selene'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Selene'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `selene.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `SELENE_TUI_CONFIG` kullanın.
