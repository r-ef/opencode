...
Ajan becerileri, selene'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
selene su konumlari tarar:

- Proje konfigurasyonu: `.selene/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/selene/skills/<name>/SKILL.md`
...
Proje yerel yollarinda selene, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.selene/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/selene/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.selene/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
selene, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`selene.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`selene.json` icinde):
