---
title: Sağlayıcılar
description: selene'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

selene, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı selene yapılandırmanızda yapılandırın.
...
`~/.local/share/selene/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları selene'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Selene Zen, selene ekibi tarafından sağlanan modellerin bir listesidir.
selene ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Selene Zen ile başlamanızı öneririz.
:::
...
selene'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Selene Go

Selene Go, selene ile iyi çalıştığı test edilmiş ve doğrulanmış, selene ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Selene Go`'yu seçin ve [selene.run/auth](https://selene.run/zen) adresine gidin.
...
Amazon Bedrock'u selene ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   selene'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `selene.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi selene'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   selene'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Selene yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in selene auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
selene, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. selene'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`selene.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [selene-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`selene.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi selene'la kullanmak için:
...
Google Vertex AI'yi selene ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları selene çalıştırılırken ayarlayın.
...
Helicone'un selene aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her selene görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [selene-helicone-session](https://github.com/H2Shami/selene-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her selene görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
selene'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla selene'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla selene'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini selene için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/selene)'a bakın.
:::
...
Ollama Cloud'u selene ile kullanmak için:
...
3. selene'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: selene'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Selene Zen, selene ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Selene Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Selene Zen**'i arayın.

   ```txt
   /connect
   ```

3. selene API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   selene yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca selene yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Selene ile kullanmak için:
...
Modelleri selene yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı selene ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `selene.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları selene'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `selene auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için selene yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, selene yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
