...
selene'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
selene, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
selene, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

selene iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `selene.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse selene modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global selene yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
selene yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Selene Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `selene/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `selene models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/selene).
:::
