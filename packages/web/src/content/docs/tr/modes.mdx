...
:::caution
Modlar artık selene yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

selene'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya selene yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
selene iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.selene/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`selene.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, selene modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global selene yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.selene/modes/` veya genel modlar için `~/.config/selene/modes/`'de mod dosyaları oluşturun:
