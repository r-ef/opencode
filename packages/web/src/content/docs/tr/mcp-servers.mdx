...
_Model Bağlam Protokolü_ veya MCP kullanarak selene'a harici araçlar ekleyebilirsiniz. selene hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [selene Config](https://selene.run/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/selene` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
selene, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde selene şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa selene, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `selene mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından selene, belirteçleri `~/.local/share/selene/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra selene'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi selene yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve selene'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
