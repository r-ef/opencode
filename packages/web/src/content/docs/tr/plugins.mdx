---
title: Eklentiler
description: Selene'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Selene'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Selene'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.selene/plugins/` - Proje düzeyinde eklentiler
- `~/.config/selene/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/selene/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/selene/selene.json`)
2. Project config (`selene.json`)
3. Global eklenti dizini (`~/.config/selene/plugins/`)
4. Proje eklenti dizini (`.selene/plugins/`)
...
Selene bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Selene SDK istemcisi.
...
Selene'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Selene masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Selene'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Selene'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Selene'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Selene'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://selene.run/docs/sdk) bakın.
