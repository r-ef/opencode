...
Selene Go, popüler açık kodlama modellerine güvenilir erişim sağlayan **aylık 10$** tutarında düşük maliyetli bir aboneliktir.

:::note
Selene Go şu anda beta aşamasındadır.
:::

Go, Selene içindeki diğer sağlayıcılar gibi çalışır. Selene Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Selene'u kullanmak için buna ihtiyacınız yoktur.
...
:::tip
Selene ile iyi çalışan seçkin bir model ve sağlayıcı grubunu test ettik.
:::
...
Selene Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır

Selene Go, Selene'daki diğer herhangi bir sağlayıcı gibi çalışır.

1. **<a href={console}>Selene Zen</a>**'de oturum açın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Selene Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanılabilen modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Çalışma alanı başına yalnızca bir üye Selene Go'ya abone olabilir.
:::
...
Selene Go aşağıdaki sınırları içerir:
...
Selene Go, **aylık 10$** tutarında bir abonelik planıdır. Aşağıda **1M token başına** fiyatlar yer almaktadır.
...
Selene yapılandırmanızdaki [model kimliği](/docs/config/#models), `selene-go/<model-id>` biçimini kullanır. Örneğin, Kimi K2.5 için yapılandırmanızda `selene-go/kimi-k2.5` kullanırsınız.
...
Selene Go'yu şu amaçlarla oluşturduk:
...
4. Selene ile başka herhangi bir sağlayıcıyı kullanmanıza da izin vererek **kilitlenmeyi önlemek**.
