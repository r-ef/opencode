...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. selene yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
selene içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `SELENE_EXPERIMENTAL_LSP_TOOL=true` (veya `SELENE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca selene provider kullanılırken veya `SELENE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

selene başlatırken etkinleştirmek için:
