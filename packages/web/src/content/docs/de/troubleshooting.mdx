...
Wenn Selene Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\selene\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `selene --log-level DEBUG`.

---

## Speicher

selene speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/selene/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\selene`
...
Selene Desktop startet im Hintergrund einen lokalen Selene-Server (`selene-cli`-Sidecar).
...
- Nur macOS: `Selene`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/selene/selene.jsonc` (oder `~/.config/selene/selene.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/selene/selene.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\selene\selene.jsonc`
...
Selene kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/selene/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\selene\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Selene ihn neu aufbauen kann.

1. Beende Selene Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/selene`
- **Linux**: loesche `~/.cache/selene` (oder fuehre aus: `rm -rf ~/.cache/selene`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\selene`

3. Starte Selene Desktop neu.

---

### Server-Verbindungsprobleme beheben

Selene Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `selene.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `SELENE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `SELENE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Selene Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Selene-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Selene Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Selene in den OS-Einstellungen aktiviert sind, und
...
1. Beende Selene Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Selene Desktop):

- `selene.settings.dat` (desktop default server URL)
- `selene.global.dat` und `selene.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Selene hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `selene models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\selene`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. selene installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\selene`

2. Starte selene neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt selene, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
selene erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
