---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Selene zu erweitern.
---

Mit Plugins können Sie Selene erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Selene zu ändern.
...
- `.selene/plugins/` – Plugins auf Projektebene
- `~/.config/selene/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/selene/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/selene/selene.json`)
2. Projektkonfiguration (`selene.json`)
3. Globales Plugin-Verzeichnis (`~/.config/selene/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.selene/plugins/`)
...
Selene führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Selene-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Selene erweitern können.
...
:::note
Wenn Sie die Selene-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Selene `.env`-Dateien liest:
...
Plugins können Selene auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Selene aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Selene zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://selene.run/docs/sdk).
