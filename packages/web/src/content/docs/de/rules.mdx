---
title: Regeln
description: Lege eigene Anweisungen fuer selene fest.
---

Du kannst selene ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in selene den Befehl `/init` aus.
...
So kann selene sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht selene sie zu erweitern.
...
selene kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/selene/AGENTS.md` speichern.
Diese gelten in allen selene-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Selene dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/selene/AGENTS.md` existiert)
...
Beim Start sucht selene in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/selene/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `selene.json` oder in `~/.config/selene/selene.json` angeben.
...
selene parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit selene.json

Empfohlen ist das `instructions`-Feld in der `selene.json`:
...
Du kannst selene explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `selene.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
