...
Selene Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte Open-Coding-Modelle bietet.

:::note
Selene Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Anbieter in Selene. Du abonnierst Selene Go und erhältst deinen API-Schlüssel. Es ist **völlig optional** und du musst es nicht nutzen, um Selene zu verwenden.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Anbietern getestet, die gut mit Selene funktionieren.
:::
...
Selene Go gibt dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

Selene Go funktioniert wie jeder andere Anbieter in Selene.

1. Du meldest dich bei **<a href={console}>Selene Zen</a>** an, abonnierst Go und kopierst deinen API-Schlüssel.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Selene Go` und fügst deinen API-Schlüssel ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Selene Go abonnieren.
:::
...
Selene Go beinhaltet die folgenden Limits:
...
Selene Go ist ein Abonnementplan für **10 $/Monat**. Unten stehen die Preise **pro 1 Mio. Token**.
...
| ------------ | ------------ | ------------------------------------------------ | --------------------------- |
| GLM-5        | glm-5        | `https://selene.run/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| Kimi K2.5    | kimi-k2.5    | `https://selene.run/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| MiniMax M2.5 | minimax-m2.5 | `https://selene.run/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |

Die [Modell-ID](/docs/config/#models) in deiner Selene-Konfiguration verwendet das Format `selene-go/<model-id>`. Zum Beispiel würdest du für Kimi K2.5 `selene-go/kimi-k2.5` in deiner Konfiguration verwenden.
...
Wir haben Selene Go erstellt, um:
...
4. **Keinen Lock-in** zu haben, indem wir dir ermöglichen, jeden anderen Anbieter ebenfalls mit Selene zu nutzen.
