---
title: Server
description: Nutze den selene-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `selene serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein selene-Client nutzen kann.
...
Setze `SELENE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `selene`, kann aber mit `SELENE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `selene serve` und `selene web`.
...
Wenn du `selene` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den selene-Server, um selene programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `selene serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `selene serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Selene-[IDE](/docs/ide)-Plugins.
...
Der selene-Server stellt folgende APIs bereit.
