...
Mit Selene kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Selene das `selene`-Theme.
...
Selene liefert mehrere eingebaute Themes mit.
...
- Selene optisch ans Terminal anpassen wollen
...
Selene unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/selene/themes/*.json` oder `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.selene/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.selene/themes/*.json`
