...
Agent Skills erlauben Selene, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Selene sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Selene vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.selene/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/selene/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.selene/skills/git-release/SKILL.md` zum Beispiel so:
...
Selene listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `selene.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `selene.json`):
