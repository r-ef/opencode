---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Selene.
---

import config from "../../../../config.mjs"
export const console = config.console

Selene bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Selene-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/selene/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Selene-konfigurationen.
...
Selene Zen er en liste over modeller leveret af Selene-teamet, som er blevet testet og verificeret til at fungere godt med Selene. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Selene Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg selene og gå til [selene.run/auth](https://selene.run/auth).
...
Det fungerer som alle andre udbydere i Selene og er helt valgfrit at bruge.

---

## Selene Go

Selene Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Selene-teamet, som er testet og verificeret til at fungere godt med Selene.

1. Kør kommandoen `/connect` i TUI, vælg `Selene Go`, og gå til [selene.run/auth](https://selene.run/zen).
...
Sådan bruger du Amazon Bedrock med Selene:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører selene:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `selene.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Selene understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at selene skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem selene-konfigurationen.
...
gemme tokenet i selene auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Selene bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Selene
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`selene.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [selene-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `selene.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Selene:
...
Sådan bruger du Google Vertex AI med Selene:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem selene, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [selene-helicone-session](https://github.com/H2Shami/selene-helicone-session) plugin for automatisk at logge hver Selene-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Selene-samtale opført som en separat session.
...
Du kan konfigurere selene til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere selene til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere selene til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Selene. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/selene) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Selene:
...
3. Kopier API-nøglen til brug i Selene.
...
6. **Vigtigt**: Før du bruger skymodeller i Selene, skal du hente modelinformationen lokalt:
...
Selene Zen er en liste over testede og verificerede modeller leveret af Selene-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Selene Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Selene Zen**.

   ```txt
   /connect
   ```

3. Indtast Selene API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem selene-konfigurationen.
...
5. Du kan også tilpasse dem gennem selene-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Selene:
...
Du kan også tilpasse modeller gennem selene-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med selene. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `selene.json`-filen i projektkataloget:
...
`limit`-felterne lader Selene forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `selene auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek selene-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i selene-konfigurationen.
