...
Med Selene kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Selene vårt eget `selene`-tema.
...
Selene støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/selene/themes/*.json` eller `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.selene/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.selene/themes/*.json`
