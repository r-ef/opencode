...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i selene-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i selene giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem selene-konfigurationen.
...
selene leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.selene/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `selene.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger selene modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale selene-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.selene/modes/` for projektspecifikke tilstande eller `~/.config/selene/modes/` for globale tilstande:
