...
Agentferdigheter lar Selene oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Selene søker etter disse stedene:

- Prosjektkonfigurasjon: `.selene/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/selene/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Selene opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.selene/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/selene/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.selene/skills/git-release/SKILL.md` slik:
...
Selene viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `selene.json`:
...
**For innebygde agenter** (i `selene.json`):
