...
For at fejlfinde problemer med Selene, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\selene\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `selene --log-level DEBUG`.

---

## Lagring

selene gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/selene/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\selene`
...
Selene Desktop kører en lokal Selene-server (`selene-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Selene`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/selene/selene.jsonc` (eller `~/.config/selene/selene.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/selene/selene.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\selene\selene.jsonc`
...
Selene kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/selene/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\selene\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Selene kan genopbygge den.

1. Afslut Selene Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/selene`
- **Linux**: slet `~/.cache/selene` (eller kør `rm -rf ~/.cache/selene`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\selene`

3. Start Selene Desktop på ny.

---

### Løs problemer med serverforbindelse

Selene Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `selene.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `SELENE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `SELENE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Selene Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Selene.

---

### Meddelelser vises ikke

Selene Desktop viser kun systemvarsler når:

- varsler er aktiveret for Selene i dine OS-indstillinger, og
...
1. Afslut Selene Desktop.
2. Find og slet disse filer (de findes i Selene Desktop-appens datamappe):

- `selene.settings.dat` (skrivebordsstandardserver URL)
- `selene.global.dat` og `selene.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Selene:
...
For at finde ud af hvilke modeller du har adgang til, kør `selene models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\selene`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. selene installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\selene`

2. Start selene på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge selene til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
selene vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
