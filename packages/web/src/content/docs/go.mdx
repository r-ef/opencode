...
Selene Go is a low cost **$10/month** subscription that gives you reliable access to popular open coding models.

:::note
Selene Go is currently in beta.
:::

Go works like any other provider in Selene. You subscribe to Selene Go and
...
Selene Go gives you access to these models for **$10/month**.

---

## How it works

Selene Go works like any other provider in Selene.

1. You sign in to **<a href={console}>Selene Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Selene Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Selene Go.
:::
...
Selene Go includes the following limits:
...
uses the format `selene-go/<model-id>`. For example, for Kimi K2.5, you would
use `selene-go/kimi-k2.5` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Selene as well.
