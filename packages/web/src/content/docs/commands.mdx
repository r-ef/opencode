...
Create `.selene/commands/test.md`:
...
You can add custom commands through the Selene config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Selene [config](/docs/config):
...
selene includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
