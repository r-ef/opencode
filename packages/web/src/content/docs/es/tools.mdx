...
Las herramientas permiten que LLM realice acciones en su código base. Selene viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Selene.
...
:::note
Esta herramienta solo está disponible cuando `SELENE_EXPERIMENTAL_LSP_TOOL=true` (o `SELENE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Selene o cuando la variable de entorno `SELENE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Selene:

```bash
SELENE_ENABLE_EXA=1 selene
```
