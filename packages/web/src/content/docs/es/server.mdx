---
title: Servidor
description: Interactuar con el servidor selene a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `selene serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente selene puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `selene.local` |
...
Configure `SELENE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `selene`, o configure `SELENE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `selene serve` como a `selene web`.
...
Cuando ejecuta `selene`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor selene para interactuar con selene mediante programación.
:::

Esta arquitectura permite que selene admita múltiples clientes y le permite interactuar con selene mediante programación.

Puede ejecutar `selene serve` para iniciar un servidor independiente. Si tienes el
selene TUI ejecutándose, `selene serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Selene [IDE](/docs/ide).
...
El servidor selene expone las siguientes API.
