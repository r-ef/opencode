...
description: Cree herramientas que LLM pueda llamar en selene.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de selene como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.selene/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/selene/tools/`.
...
```ts title=".selene/tools/database.ts" {1}
...
```ts title=".selene/tools/project.ts" {8}
import { tool } from "@selene-ai/plugin"
...
```python title=".selene/tools/add.py"
import sys
...
```ts title=".selene/tools/python-add.ts" {10}
import { tool } from "@selene-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".selene/tools/add.py")
