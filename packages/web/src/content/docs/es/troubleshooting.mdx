...
Para depurar problemas con Selene, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/selene/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\selene\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `selene --log-level DEBUG`.

---

## Almacenamiento

selene almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/selene/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\selene`
...
Selene Desktop ejecuta un servidor Selene local (el sidecar `selene-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Selene` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/selene/selene.jsonc` (o `~/.config/selene/selene.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/selene/selene.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\selene\selene.jsonc`
...
Selene también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/selene/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\selene\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Selene pueda reconstruirlo.

1. Salga de Selene Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/selene`
- **Linux**: eliminar `~/.cache/selene` (o ejecutar `rm -rf ~/.cache/selene`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\selene`

3. Reinicie el escritorio Selene.

---

### Solucionar problemas de conexión del servidor

Selene Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `selene.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `SELENE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `SELENE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Selene requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Selene.

---

### Notificaciones que no se muestran

Selene Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Selene en la configuración de su sistema operativo, y
...
1. Salga del escritorio Selene.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Selene):

- `selene.settings.dat` (URL del servidor predeterminado de escritorio)
- `selene.global.dat` y `selene.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/selene/issues**](https://github.com/anomalyco/selene/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**selene.run/discord**](https://selene.run/discord)
...
3. Asegúrese de tener la última versión con `selene upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `selene models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\selene`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. selene instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\selene`

2. Reinicie selene para reinstalar los paquetes del proveedor más recientes.

Esto obligará a selene a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
selene detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
