...
Las habilidades del agente permiten a Selene descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Selene busca estas ubicaciones:

- Configuración del proyecto: `.selene/skills/<name>/SKILL.md`
- Configuración global: `~/.config/selene/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Selene sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.selene/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/selene/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.selene/skills/git-release/SKILL.md` así:
...
Selene enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `selene.json`:
...
**Para agentes integrados** (en `selene.json`):
