---
title: Complementos
description: Escriba sus propios complementos para extender Selene.
---

Los complementos le permiten extender Selene al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Selene.
...
- `.selene/plugins/` - Complementos a nivel de proyecto
- `~/.config/selene/plugins/` - Complementos globales
...
```json title="selene.json"
{
  ...
  "plugin": ["selene-helicone-session", "selene-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/selene/node_modules/`.
...
1. Configuración global (`~/.config/selene/selene.json`)
2. Configuración del proyecto (`selene.json`)
3. Directorio global de complementos (`~/.config/selene/plugins/`)
4. Directorio de complementos del proyecto (`.selene/plugins/`)
...
```json title=".selene/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Selene ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".selene/plugins/my-plugin.ts"
...
```js title=".selene/plugins/example.js"
...
- `client`: Un cliente SDK selene para interactuar con la IA.
...
import type { Plugin } from "@selene-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar selene.
...
```js title=".selene/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "selene"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Selene, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que selene lea archivos `.env`:

```javascript title=".selene/plugins/env-protection.js"
...
```javascript title=".selene/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a selene:

```ts title=".selene/plugins/custom-tools.ts"
import { type Plugin, tool } from "@selene-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que selene puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para selene junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://selene.run/docs/sdk) para obtener más detalles.
...
```ts title=".selene/plugins/compaction.ts"
import type { Plugin } from "@selene-ai/plugin"
...
```ts title=".selene/plugins/custom-compaction.ts"
import type { Plugin } from "@selene-ai/plugin"
