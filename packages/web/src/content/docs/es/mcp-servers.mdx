...
Puede agregar herramientas externas a Selene utilizando el _Model Context Protocol_ o MCP. Selene admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Selene Config](https://selene.run/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/selene`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Selene maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Selene:
...
Si el servidor requiere autenticación, Selene le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `selene mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Selene almacenará los tokens de forma segura en `~/.local/share/selene/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Selene, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Selene como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Selene a su cuenta Sentry.
