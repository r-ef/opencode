...
Con Selene puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Selene usa nuestro propio tema `selene`.
...
Selene viene con varios temas integrados.
...
- Quiere que Selene coincida con la apariencia de su terminal
...
Selene admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/selene/themes/*.json` o `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.selene/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.selene/themes/*.json`
