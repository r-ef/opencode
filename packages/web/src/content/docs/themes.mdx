...
With Selene you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Selene uses our own `selene` theme.
...
Selene comes with several built-in themes.
...
- Want Selene to match their terminal's appearance
...
Selene supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/selene/themes/*.json` or `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.selene/themes/*.json`
4. **Current working directory** - Defined in `./.selene/themes/*.json`
