---
title: Server
description: Interact with selene server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `selene serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an selene client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `selene.local` |
...
Set `SELENE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `selene`, or set `SELENE_SERVER_USERNAME` to override it. This applies to both `selene serve` and `selene web`.
...
When you run `selene` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the selene server to interact with selene programmatically.
:::

This architecture lets selene support multiple clients and allows you to interact with selene programmatically.

You can run `selene serve` to start a standalone server. If you have the
selene TUI running, `selene serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Selene [IDE](/docs/ide) plugins.
...
The selene server exposes the following APIs.
