...
Le skill degli agenti permettono a Selene di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Selene cerca in queste posizioni:

- Config di progetto: `.selene/skills/<name>/SKILL.md`
- Config globale: `~/.config/selene/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Selene risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.selene/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/selene/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.selene/skills/git-release/SKILL.md` cosi':
...
Selene elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `selene.json`:
...
**Per agenti integrati** (in `selene.json`):
