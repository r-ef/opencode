---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Selene.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Selene fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Selene avvia la TUI per la directory corrente.
...
Quando usi la TUI di Selene, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Selene. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `selene.json`, che configura il comportamento di server/runtime.
...
Usa `SELENE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
