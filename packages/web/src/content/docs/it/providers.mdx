---
title: Provider
description: Usa qualsiasi provider LLM in Selene.
---

import config from "../../../../config.mjs"
export const console = config.console

Selene usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Selene.
...
Selene Zen e una lista di modelli forniti dal team Selene, testati e
verificati per funzionare bene con Selene. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Selene Zen.
:::

1. Esegui `/connect` nella TUI, seleziona selene e vai su [selene.run/auth](https://selene.run/auth).
...
Funziona come qualsiasi altro provider in Selene ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Selene:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui selene:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `selene.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Selene non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché selene funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di selene.
...
salvare il token nello storage di auth di selene.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Selene usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Selene
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `selene.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [selene-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configurazione

Personalizza tramite `selene.json`:
...
Per usare il tuo abbonamento GitHub Copilot con selene:
...
Per usare Google Vertex AI con Selene:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui selene.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da selene, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [selene-helicone-session](https://github.com/H2Shami/selene-helicone-session) per loggare automaticamente ogni conversazione di Selene come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Selene elencata come sessione separata.
...
Puoi configurare selene per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare selene per usare modelli locali tramite LM Studio.
...
Puoi configurare selene per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Selene. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/selene) per dettagli.
:::
...
Per usare Ollama Cloud con Selene:
...
3. Copia la chiave API da usare in Selene.
...
6. **Importante**: Prima di usare modelli cloud in Selene, devi scaricare le informazioni del modello localmente:
...
Selene Zen è una lista di modelli testati e verificati forniti dal team Selene. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Selene Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Selene Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Selene.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di selene.
...
5. Puoi anche personalizzarli tramite la configurazione di selene. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Selene:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di selene. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con selene. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `selene.json` nella directory del progetto:
...
I campi `limit` permettono a Selene di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `selene auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di selene e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config selene.
