...
Con Selene puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Selene usa il tema `selene`.
...
Selene include diversi temi integrati.
...
- Vuole che Selene corrisponda all'aspetto del terminale
...
Selene supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/selene/themes/*.json` o `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.selene/themes/*.json`
4. **Directory di lavoro corrente** - in `./.selene/themes/*.json`
