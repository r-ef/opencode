---
title: Plugin
description: Scrivi plugin per estendere Selene.
---

I plugin ti permettono di estendere Selene agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Selene.
...
- `.selene/plugins/` - plugin a livello progetto
- `~/.config/selene/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/selene/node_modules/`.
...
1. Config globale (`~/.config/selene/selene.json`)
2. Config di progetto (`selene.json`)
3. Directory plugin globale (`~/.config/selene/plugins/`)
4. Directory plugin di progetto (`.selene/plugins/`)
...
Selene esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di selene per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere selene.
...
:::note
Se usi l'app desktop di Selene, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a selene di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a selene:
...
L'helper `tool` crea uno strumento personalizzato che selene può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in selene insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://selene.run/docs/sdk) per i dettagli.
