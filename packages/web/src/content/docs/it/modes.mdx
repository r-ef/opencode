...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di selene. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in selene ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di selene.
...
selene include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.selene/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `selene.json`:
...
- Globale: `~/.config/selene/modes/`
- Progetto: `.selene/modes/`
...
Se non viene specificata alcuna temperatura, selene usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di selene sia per quella specifica del progetto.
...
Crea file di modalita in `.selene/modes/` per modalita specifiche del progetto oppure in `~/.config/selene/modes/` per modalita globali:
