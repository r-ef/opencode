...
In Selene esistono due tipi di agenti: agenti primari e subagenti.
...
Selene include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Selene include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Selene include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `selene.json`:
...
- Globale: `~/.config/selene/agents/`
- Per progetto: `.selene/agents/`
...
Se non specifichi una temperature, Selene usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Selene sia per la config specifica del progetto.
...
L'ID modello nella configurazione Selene usa il formato `provider/model-id`. Per esempio, se stai usando [Selene Zen](/docs/zen), useresti `selene/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `selene models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/selene).
:::
