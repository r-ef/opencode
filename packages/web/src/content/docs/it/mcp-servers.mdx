...
Puoi aggiungere strumenti esterni a Selene usando il _Model Context Protocol_, o MCP. Selene supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Selene Config](https://selene.run/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/selene`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Selene gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Selene:
...
Se il server richiede autenticazione, Selene ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `selene mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Selene memorizzera' i token in modo sicuro in `~/.local/share/selene/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Selene insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Selene come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Selene al tuo account Sentry.
