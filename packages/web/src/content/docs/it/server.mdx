---
title: Server
description: Interagisci con il server selene via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `selene serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client selene.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `selene.local` |
...
Imposta `SELENE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `selene`, oppure imposta `SELENE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `selene serve` sia per `selene web`.
...
Quando esegui `selene` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server selene per interagire con selene in modo programmatico.
:::

Questa architettura permette a selene di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `selene serve` per avviare un server standalone. Se la TUI di selene e gia in esecuzione, `selene serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Selene.
...
Il server selene espone le seguenti API.
