...
You can add external tools to Selene using the _Model Context Protocol_, or MCP. Selene supports both local and remote servers.
...
You can define MCP servers in your [Selene Config](https://selene.run/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/selene` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Selene automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Selene will:
...
If the server requires authentication, Selene will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `selene mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Selene will store the tokens securely in `~/.local/share/selene/mcp-auth.json`.
...
Your MCPs are available as tools in Selene, alongside built-in tools. So you can manage them through the Selene config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Selene to your Sentry account.
