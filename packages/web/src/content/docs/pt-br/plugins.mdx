---
title: Plugins
description: Escreva seus próprios plugins para estender o selene.
---

Plugins permitem que você estenda o selene conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do selene.
...
- `.selene/plugins/` - Plugins em nível de projeto
- `~/.config/selene/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/selene/node_modules/`.
...
1. Configuração global (`~/.config/selene/selene.json`)
2. Configuração do projeto (`selene.json`)
3. Diretório de plugins global (`~/.config/selene/plugins/`)
4. Diretório de plugins do projeto (`.selene/plugins/`)
...
O selene executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do selene para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o selene.
...
:::note
Se você estiver usando o aplicativo desktop selene, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o selene de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao selene:
...
O helper `tool` cria uma ferramenta personalizada que o selene pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o selene junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://selene.run/docs/sdk) para detalhes.
