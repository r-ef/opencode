...
As habilidades do agente permitem que o selene descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O selene pesquisa nesses locais:

- Configuração do projeto: `.selene/skills/<name>/SKILL.md`
- Configuração global: `~/.config/selene/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o selene sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.selene/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/selene/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.selene/skills/git-release/SKILL.md` assim:
...
O selene lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `selene.json`:
...
**Para agentes embutidos** (em `selene.json`):
