...
Com o selene, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o selene usa nosso próprio tema `selene`.
...
O selene vem com vários temas embutidos.
...
- Querem que o selene corresponda à aparência do seu terminal
...
O selene suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/selene/themes/*.json` ou `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.selene/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.selene/themes/*.json`
