---
title: Servidor
description: Interaja com o servidor selene via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `selene serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente selene pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `selene.local` |
...
Defina `SELENE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `selene`, ou defina `SELENE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `selene serve` quanto ao `selene web`.

```bash
SELENE_SERVER_PASSWORD=your-password selene serve
```
...
Quando você executa `selene`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor selene para interagir com o selene programaticamente.
:::

Essa arquitetura permite que o selene suporte múltiplos clientes e permite que você interaja com o selene programaticamente.

Você pode executar `selene serve` para iniciar um servidor autônomo. Se você tiver o TUI do selene em execução, `selene serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do selene [IDE](/docs/ide).
...
O servidor selene expõe as seguintes APIs.
