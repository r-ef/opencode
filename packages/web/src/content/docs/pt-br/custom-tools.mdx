...
description: Crie ferramentas que o LLM pode chamar no selene.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do selene, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.selene/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/selene/tools/`.
