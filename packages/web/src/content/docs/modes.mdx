...
:::caution
Modes are now configured through the `agent` option in the selene config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in selene allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the selene config.
...
selene comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.selene/plans/*.md` to detail the plan itself
...
Configure modes in your `selene.json` config file:
...
both the global selene config and the project specific config.
...
Create mode files in `.selene/modes/` for project-specific modes or `~/.config/selene/modes/` for global modes:
