---
title: Plugins
description: Écrivez vos propres plugins pour étendre Selene.
---

Les plugins vous permettent d'étendre Selene en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Selene.
...
- `.selene/plugins/` - Plugins au niveau du projet
- `~/.config/selene/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/selene/node_modules/`.
...
1. Configuration globale (`~/.config/selene/selene.json`)
2. Configuration du projet (`selene.json`)
3. Répertoire global des plugins (`~/.config/selene/plugins/`)
4. Répertoire des plugins du projet (`.selene/plugins/`)
...
Selene exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK selene pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre selene.
...
:::note
Si vous utilisez l'application de bureau Selene, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez selene de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à selene :
...
L'assistant `tool` crée un outil personnalisé que selene peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour selene aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://selene.run/docs/sdk) pour plus de détails.
