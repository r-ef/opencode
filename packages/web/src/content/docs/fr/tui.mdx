---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Selene.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Selene fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Selene démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Selene TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Selene. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `selene.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `SELENE_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
