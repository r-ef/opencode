...
Avec Selene, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Selene utilise notre propre thème `selene`.
...
Selene est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Selene corresponde à l'apparence de leur terminal
...
Selene prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/selene/themes/*.json` ou `$XDG_CONFIG_HOME/selene/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.selene/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.selene/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/selene/themes
vim ~/.config/selene/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .selene/themes
vim .selene/themes/my-theme.json
```
...
{
  "$schema": "https://selene.run/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
