---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Selene.
---

import config from "../../../../config.mjs"
export const console = config.console

Selene utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Selene.
...
en `~/.local/share/selene/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Selene
configuration.
...
Selene Zen est une liste de modèles fournis par l'équipe Selene qui ont été
testé et vérifié pour fonctionner correctement avec Selene. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Selene Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez selene et dirigez-vous vers [selene.run/auth](https://selene.run/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Selene et son utilisation est totalement facultative.

---

## Selene Go

Selene Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Selene qui ont été
testé et vérifié pour fonctionner correctement avec Selene.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Selene Go` et rendez-vous sur [selene.run/auth](https://selene.run/zen).
...
Pour utiliser Amazon Bedrock avec Selene :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de selene :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY selene

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile selene

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX selene
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `selene.json` :

```json title="selene.json"
{
  "$schema": "https://selene.run/config.json",
  "provider": {
    "amazon-bedrock": {
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Selene n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que selene fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX selene
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX selene
   ```
...
Vous pouvez également ajouter des modèles via votre configuration selene.
...
pour stocker le jeton dans le stockage d'authentification selene.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Selene utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Selene
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `selene.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [selene-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuration

Personnalisez via `selene.json` :
...
{
  "$schema": "https://selene.run/config.json",
  "plugin": ["@gitlab/selene-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec selene :
...
Pour utiliser Google Vertex AI avec Selene :
...
Définissez-les lors de l'exécution de selene.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id selene
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via selene, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/selene/selene.jsonc"
{
  "$schema": "https://selene.run/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [selene-helicone-session](https://github.com/H2Shami/selene-helicone-session) pour enregistrer automatiquement chaque conversation Selene en tant que session dans Helicone.

```bash
npm install -g selene-helicone-session
```
...
```json title="selene.json"
{
  "plugin": ["selene-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Selene répertoriée comme une session distincte.
...
Vous pouvez configurer selene pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="selene.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://selene.run/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer selene pour utiliser des modèles locaux via LM Studio.

```json title="selene.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://selene.run/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer selene pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Selene. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/selene) pour plus de détails.
:::

```json title="selene.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://selene.run/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Selene :
...
3. Copiez la clé API à utiliser dans Selene.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Selene, vous devez extraire les informations du modèle localement :
...
Selene Zen est une liste de modèles testés et vérifiés fournie par l'équipe Selene. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Selene Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Selene Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Selene API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration selene.
...
5. Vous pouvez également les personnaliser via votre configuration selene. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' selene
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group selene
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Selene :
...
Vous pouvez également personnaliser les modèles via votre configuration selene. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec selene. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in selene.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `selene.json` dans le répertoire de votre projet :

   ```json title="selene.json" ""myprovider"" {5-15}
   {
     "$schema": "https://selene.run/config.json",
     "provider": {
       "myprovider": {
...
```json title="selene.json" {9,11,17-20}
{
  "$schema": "https://selene.run/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Selene de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `selene auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration selene et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration selene.
