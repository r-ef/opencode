...
Il existe deux types d'agents dans Selene ; agents primaires et sous-agents.
...
Selene est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Selene est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Selene est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `selene.json` :
...
- Global : `~/.config/selene/agents/`
- Par projet : `.selene/agents/`

```markdown title="~/.config/selene/agents/review.md"
---
...
Si aucune température n'est spécifiée, Selene utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Selene et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Selene utilise le format `provider/model-id`. Par exemple, si vous utilisez [Selene Zen](/docs/zen), vous utiliserez `selene/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="selene.json" {3-6,9-12}
{
  "$schema": "https://selene.run/config.json",
  "tools": {
    "write": true,
...
```json title="selene.json" {3-5,8-10}
{
  "$schema": "https://selene.run/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `selene models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/selene).
:::
...
```markdown title="~/.config/selene/agents/docs-writer.md"
---
...
```markdown title="~/.config/selene/agents/security-auditor.md"
---
