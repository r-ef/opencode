---
title: SDK
description: Client JS de type sécurisé pour le serveur selene.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK selene JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler selene par programme.
...
Créez une instance de selene :

```javascript
import { createSelene } from "@selene-ai/sdk"

const { client } = await createSelene()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `selene.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${selene.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de selene, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@selene-ai/sdk"
```
