---
title: Serveur
description: Interagissez avec le serveur selene via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `selene serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client selene peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `selene.local` |
...
Définissez `SELENE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `selene`, ou définissez `SELENE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `selene serve` et à `selene web`.
...
Lorsque vous exécutez `selene`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur selene pour interagir avec selene par programmation.
:::

Cette architecture permet à selene de prendre en charge plusieurs clients et vous permet d'interagir avec selene par programme.

Vous pouvez exécuter `selene serve` pour démarrer un serveur autonome. Si vous avez le
selene TUI en cours d'exécution, `selene serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Selene [IDE](/docs/ide).
...
Le serveur selene expose les API suivantes.
