---
title: Configuration
description: Utilisation de la configuration JSON Selene.
---

Vous pouvez configurer Selene à l'aide d'un fichier de configuration JSON.

---

## Format

Selene prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="selene.jsonc"
{
...
1. **Configuration distante** (à partir de `.well-known/selene`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/selene/selene.json`) - préférences utilisateur
3. **Configuration personnalisée** (`SELENE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`selene.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.selene`** - agents, commandes, plugins
6. **Configuration en ligne** (`SELENE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.selene` et `~/.config/selene` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/selene`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/selene"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale Selene dans `~/.config/selene/selene.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/selene/tui.json`.
...
Ajoutez `selene.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque Selene démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `SELENE_CONFIG`.

```bash
export SELENE_CONFIG=/path/to/my/custom-config.json
selene run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `SELENE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.selene`, et devrait suivre la même structure.

```bash
export SELENE_CONFIG_DIR=/path/to/my/config-directory
selene run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.selene`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`selene.run/config.json`**](https://selene.run/config.json).

La configuration TUI utilise [**`selene.run/tui.json`**](https://selene.run/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://selene.run/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
Utilisez `SELENE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `selene.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `selene serve` et `selene web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Selene.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `selene.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Selene via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Selene essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/selene/agents/` ou `.selene/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Selene reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`selene run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/selene/commands/` ou `.selene/commands/`. [En savoir plus ici](/docs/commands).
...
Selene téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, selene **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Selene avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.selene/plugins/` ou `~/.config/selene/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="selene.json"
{
  "$schema": "https://selene.run/config.json",
  "plugin": ["selene-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre Selene à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
