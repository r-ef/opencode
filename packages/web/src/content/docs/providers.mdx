---
title: Providers
description: Using any LLM provider in Selene.
---

import config from "../../../config.mjs"
export const console = config.console

Selene uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Selene config.
...
You can customize the providers through the `provider` section in your Selene
config.
...
Selene Zen is a list of models provided by the Selene team that have been
tested and verified to work well with Selene. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Selene Zen.
:::

1. Run the `/connect` command in the TUI, select `Selene Zen`, and head to [selene.run/auth](https://selene.run/zen).
...
It works like any other provider in Selene and is completely optional to use.

---

## Selene Go

Selene Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Selene team that have been
tested and verified to work well with Selene.

1. Run the `/connect` command in the TUI, select `Selene Go`, and head to [selene.run/auth](https://selene.run/zen).
...
To use Amazon Bedrock with Selene:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running selene:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `selene.json`:
...
:::info
Using your Claude Pro/Max subscription in Selene is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for selene to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your selene config.
...
Selene uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Selene
to only use your own GitLab-hosted instance, add the following to your
`selene.json` file. It is also recommended to disable session sharing.
...
More documentation on [selene-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) homepage.
...
To use your GitHub Copilot subscription with selene:
...
To use Google Vertex AI with Selene:
...
In the event you see a feature or model from Helicone that isn't configured automatically through selene, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [selene-helicone-session](https://github.com/H2Shami/selene-helicone-session) plugin to automatically log each Selene conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Selene conversation listed as a separate session.
...
You can configure selene to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure selene to use local models through LM Studio.
...
You can configure selene to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Selene. See the [Ollama integration docs](https://docs.ollama.com/integrations/selene) for details.
:::
...
To use Ollama Cloud with Selene:
...
3. Copy the API key for use in Selene.
...
6. **Important**: Before using cloud models in Selene, you must pull the model information locally:
...
Selene Zen is a list of tested and verified models provided by the Selene team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Selene Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Selene Zen**.

   ```txt
   /connect
   ```

3. Enter your Selene API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your selene config.
...
5. You can also customize them through your selene config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Selene:
...
You can also customize models through your selene config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with selene. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `selene.json` file in your project directory:
...
The `limit` fields allow Selene to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `selene auth list` to see if the credentials
...
2. For custom providers, check the selene config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your selene config.
