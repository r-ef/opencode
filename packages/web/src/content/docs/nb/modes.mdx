...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i Selene-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i Selene lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom Selene-konfigurasjonen.
...
Selene kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.selene/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `selene.json` konfigurasjonsfil:
...
- Globalt: `~/.config/selene/modes/`
- Prosjekt: `.selene/modes/`
...
Hvis ingen temperatur er spesifisert, bruker Selene modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Selene-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.selene/modes/` for prosjektspesifikke moduser eller `~/.config/selene/modes/` for globale moduser:
