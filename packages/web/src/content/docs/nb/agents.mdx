...
Det er to typer agenter i Selene; primære agenter og underagenter.
...
Selene kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Selene kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Selene kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `selene.json`:
...
- Globalt: `~/.config/selene/agents/`
- Per prosjekt: `.selene/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Selene modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Selene-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Selene-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Selene Zen](/docs/zen), vil du bruke `selene/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `selene models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/selene).
:::
