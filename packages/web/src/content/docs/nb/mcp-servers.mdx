...
Du kan legge til eksterne verktøy i Selene ved å bruke _Model Context Protocol_ eller MCP. Selene støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Selene Config](https://selene.run/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="selene.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/selene`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Selene håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Selene:
...
Hvis serveren krever autentisering, vil Selene be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `selene mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Selene lagre tokens sikkert i `~/.local/share/selene/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Selene, sammen med innebygde verktøy. Så du kan administrere dem gjennom Selene-konfigurasjonen som alle andre verktøy.
...
```json title="selene.json" {11, 14-18}
{
  "$schema": "https://selene.run/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Selene til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Selene-samtale oppført som en separat økt.
