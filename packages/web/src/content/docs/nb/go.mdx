...
Selene Go er et lavkostnadsabonnement til **$10/måned** som gir deg pålitelig tilgang til populære åpne kodemodeller.

:::note
Selene Go er for tiden i beta.
:::

Go fungerer som enhver annen leverandør i Selene. Du abonnerer på Selene Go og
...
:::tip
Vi testet en utvalgt gruppe modeller og leverandører som fungerer bra med Selene.
:::
...
Selene Go gir deg tilgang til disse modellene for **$10/måned**.

---

## Hvordan det fungerer

Selene Go fungerer som enhver annen leverandør i Selene.

1. Du logger deg inn på **<a href={console}>Selene Zen</a>**, abonnerer på Go, og
   kopierer API-nøkkelen din.
2. Du kjører kommandoen `/connect` i TUI-en, velger `Selene Go`, og limer inn
   API-nøkkelen din.
3. Kjør `/models` i TUI-en for å se listen over modeller tilgjengelig gjennom Go.

:::note
Bare ett medlem per arbeidsområde kan abonnere på Selene Go.
:::
...
Selene Go inkluderer følgende grenser:
...
Selene Go er et **$10/måned** abonnementsplan. Nedenfor er prisene **per 1M tokens**.
...
[Modell-ID-en](/docs/config/#models) i din Selene-konfigurasjon
bruker formatet `selene-go/<model-id>`. For eksempel, for Kimi K2.5, ville du
bruke `selene-go/kimi-k2.5` i konfigurasjonen din.
...
4. Ha **ingen innlåsing** ved å tillate deg å bruke hvilken som helst annen leverandør med Selene også.
