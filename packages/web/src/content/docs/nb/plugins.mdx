...
description: Skriv dine egne programtillegg for å utvide Selene.
---

Plugins lar deg utvide Selene ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Selene.
...
- `.selene/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/selene/node_modules/`.
...
1. Global konfigurasjon (`~/.config/selene/selene.json`)
2. Prosjektkonfigurasjon (`selene.json`)
3. Global plugin-katalog (`~/.config/selene/plugins/`)
4. Prosjektpluginkatalog (`.selene/plugins/`)
...
Selene kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Selene SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Selene.
...
:::note
Hvis du bruker Selene-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Selene fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Selene:
...
`tool`-hjelperen lager et tilpasset verktøy som Selene kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Selene sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://selene.run/docs/sdk) for detaljer.
