---
title: Tjener
description: Samhandle med Selene-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `selene serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Selene-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `selene.local` |
...
Sett `SELENE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `selene` som standard, eller sett `SELENE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `selene serve` og `selene web`.
...
Når du kjører `selene` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Selene-serveren til å samhandle med Selene programmatisk.
:::

Denne arkitekturen lar Selene støtte flere klienter og lar deg samhandle med Selene programmatisk.

Du kan kjøre `selene serve` for å starte en frittstående server. Hvis du har
Selene TUI kjører, vil `selene serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Selene [IDE](/docs/ide) plugins.
...
selene-serveren viser følgende APIer.
