---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Selene.
---

import config from "../../../../config.mjs"
export const console = config.console

Selene bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Selene-konfigurasjonen.
...
i `~/.local/share/selene/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Selene
konfig.
...
Selene Zen er en liste over modeller levert av Selene-teamet som har vært
testet og verifisert for å fungere godt med Selene. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Selene Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Selene Zen` og gå til [selene.run/auth](https://selene.run/zen).
...
Det fungerer som alle andre leverandører i Selene og er helt valgfritt å bruke.

---

## Selene Go

Selene Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Selene-teamet som har vært
testet og verifisert for å fungere godt med Selene.

1. Kjør kommandoen `/connect` i TUI, velg `Selene Go`, og gå til [selene.run/auth](https://selene.run/zen).
...
Slik bruker du Amazon Bedrock med Selene:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Selene:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `selene.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i selene støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Selene skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Selene-konfigurasjonen.
...
for å lagre token i Selene auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Selene bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Selene
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`selene.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [selene-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `selene.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Selene:
...
Slik bruker du Google Vertex AI med Selene:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Selene.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Selene, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [selene-helicone-session](https://github.com/H2Shami/selene-helicone-session) plugin for automatisk å logge hver Selene-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Selene-samtale oppført som en separat økt.
...
Du kan konfigurere Selene for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Selene for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Selene for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Selene. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/selene) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Selene:
...
3. Kopier API-nøkkelen for bruk i Selene.
...
6. **Viktig**: Før du bruker skymodeller i Selene, må du hente modellinformasjonen lokalt:
...
Selene Zen er en liste over testede og verifiserte modeller levert av Selene-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Selene Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Selene Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Selene API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Selene-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Selene-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Selene:
...
Du kan også tilpasse modeller gjennom Selene-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Selene. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `selene.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Selene forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `selene auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Selene-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Selene-konfigurasjonen.
