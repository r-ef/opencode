---
title: サーバー
description: HTTP 経由で Selene サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`selene serve` コマンドは、Selene クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `selene.local` |
...
HTTP 基本認証でサーバーを保護するには、`SELENE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `selene` になるか、`SELENE_SERVER_USERNAME` を設定してオーバーライドします。これは、`selene serve` と `selene web` の両方に当てはまります。
...
`selene` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Selene サーバーを使用して、プログラムで Selene と対話します。
:::

`selene serve` を実行してスタンドアロンサーバーを起動できます。すでに Selene TUI を実行している場合でも、`selene serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Selene [IDE](/docs/ide) プラグインによって使用されます。
