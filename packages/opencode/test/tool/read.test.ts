...
describe("tool.read truncation", () => {
  ...
  test("refreshes cached content after file changes", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "cached.txt"), "first")
      },
    })
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const read = await ReadTool.init()
        const file = path.join(tmp.path, "cached.txt")
        const one = await read.execute({ filePath: file }, ctx)
        expect(one.output).toContain("first")
        await Bun.write(file, "second")
        const two = await read.execute({ filePath: file }, ctx)
        expect(two.output).toContain("second")
        expect(two.output).not.toContain("first")
      },
    })
  })
