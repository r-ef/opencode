                    ...
                    Some(
                        async move {
                            ...
                            if let Some(err) = err {
                                ...
                                return Err(format!(
                                    "Failed to spawn Selene Server ({err}). Logs:\n{}",
                                    get_logs()
                                ));
                            }
...
async fn setup_server_connection(app: AppHandle) -> ServerConnection {
    ...
    ServerConnection::CLI {
        url: local_url,
        username: Some("selene".to_string()),
...
fn get_sidecar_port() -> u32 {
    option_env!("SELENE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("SELENE_PORT").ok())
...
fn opencode_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("selene").join("selene.db"))
}
