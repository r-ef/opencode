...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.selene.desktop.dev",
  beta: "ai.selene.desktop.beta",
  prod: "ai.selene.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.selene.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `selene.settings.dat` is special: it maps to the `selene.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"selene.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // selene.settings.dat → the electron settings store ("selene.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "selene.global.dat").
  const storeName = filename === "selene.settings.dat" ? "selene.settings" : filename
