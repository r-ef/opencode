...
**Only** add if it's specifically about the Desktop application or `selene web` view. **Do not** add for terminal, TUI, or general selene issues.
...
**Only** add if the issue mentions "zen" or "selene zen" or "selene black".

If the issue doesn't have "zen" or "selene black" in it then don't add zen label

#### core

Use for core server issues in `packages/selene/`, excluding `packages/selene/src/cli/cmd/tui/`.
...
TUI (`packages/selene/src/cli/cmd/tui/...`):
...
Core (`packages/selene/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for selene server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
